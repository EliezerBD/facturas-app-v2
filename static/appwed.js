/**
 * Clase principal que maneja toda la lógica del frontend.
 * Se encarga de la autenticación, búsqueda y descarga de archivos.
 */
class GmailDownloader {
    constructor() {
        // Al instanciar la clase, iniciamos los procesos básicos
        this.init();
    }

    /**
     * Inicializa la aplicación.
     * 1. Configura los eventos (clicks, teclas).
     * 2. Verifica si el usuario acaba de iniciar sesión.
     */
    init() {
        this.bindEvents();
    ...
    /**
     * Vincula eventos del DOM a funciones de la clase.
    ...
    /**
     * Analiza la URL para ver si Google nos devolvió un estado de éxito.
     * Cuando Google redirige de vuelta, a veces añade #auth_success a la URL.
     */
    parseHashParams() {
    ...
    /**
     * Verifica el estado de autenticación basado en la URL.
     * NO maneja tokens directamente por seguridad (eso lo hace el backend con cookies).
     * Solo decide qué pantalla mostrar (Login o Principal).
     */
    async checkAuthStatus() {
        ...
        if (authSuccess) {
            ...
            // Limpiar la URL para que no se vea el #auth_success (estética y seguridad)
            history.replaceState(null, null, ' ');
            this.showNotification('Conectado a Gmail correctamente', 'success');

        } else {
            // Caso 2: Carga normal. Verificamos con el backend si hay sesión activa.
            try {
    ...
    /**
     * Inicia el flujo de conexión con Google.
     * Pide al backend la URL de autorización y redirige al usuario allí.
     */
    async connectGmail() {
        try {
            this.showNotification('Conectando con Google...', 'info');

            // Pedimos al backend la URL segura de Google
            const response = await fetch('/auth/google');
            const data = await response.json();

            if (data.authUrl && data.state) {
                // Redirigimos al usuario a Google para que acepte los permisos
    ...
    /**
     * Oculta la pantalla de login y muestra el buscador.
     * Manipula las clases CSS 'hidden' para alternar visibilidad.
     */
    showMainApp() {
    ...
    /**
     * Realiza la búsqueda de emails en el backend.
     * Recoge los filtros del HTML y los envía como JSON.
     */
    async searchFiles() {
        ...
        try {
            this.showLoading(); // Mostrar spinner de carga

            console.log('Enviando búsqueda al backend...');

            // Petición POST al backend con los criterios de búsqueda
            const response = await fetch('/api/search', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                // El backend leerá la cookie 'gmail_token' automáticamente
                body: JSON.stringify({
                    search: searchTerm,
                    startDate: startDate,
                    endDate: endDate,
            ...
            if (response.ok) {
                // Si todo salió bien, mostramos los resultados
                ...
                if (response.status === 401) {
                    this.logout(); // Si es 401, la sesión caducó
                }
            }
        } catch (error) {
    ...
    /**
     * Renderiza la lista de emails en el HTML.
     * Genera HTML dinámicamente para cada email encontrado.
     */
    displayEmails(emails) {
        ...
        // Generar HTML para cada tarjeta de email
        resultsContainer.innerHTML = emails.map(email => {
            ...
            // Template string para crear la tarjeta del email
            return `
        ...
        // Agregar listeners a los nuevos checkboxes para efectos visuales
     ...
     * Recopila los datos de los checkboxes marcados y llama a downloadAsZip.
     */
    async downloadSelected() {
        ...
        // Extraemos los datos del email guardados en el atributo data-email
    ...
    /**
     * Solicita al backend generar y descargar un ZIP con los adjuntos.
     */
    async downloadAsZip(emails) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                link.click(); // Simulamos click para iniciar descarga
     ...
     * Llama al backend para borrar la cookie y notifica al usuario.
     */
    logout() {
    ...
    /**
     * Selecciona todos los checkboxes de la lista.
     */
    selectAll() {
        document.querySelectorAll('.file-checkbox').forEach(checkbox => {
    ...
    /**
     * Desmarca todos los checkboxes de la lista.
     */
    clearSelection() {
        document.querySelectorAll('.file-checkbox').forEach(checkbox => {
    ...
    /**
     * Muestra un spinner de carga mientras se buscan los emails.
     */
    showLoading() {
     ...
     * Crea un elemento HTML temporal y lo elimina después de 5 segundos.
     */
    showNotification(message, type = 'info') {
        const container = document.getElementById('notification-container');

        const notification = document.createElement('div');
        // Clases de Tailwind para estilos según el tipo (éxito, error, info)
...
// Estas funciones actúan como puente hacia la instancia de la clase
